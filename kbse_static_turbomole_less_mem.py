...
def matvec(bse, r):
    ...
    for kL in range(nkpts):
        kLij, eps_body_inv, kidx_r = make_imds(bse.gw, kL)
        Loo = kLij[:,:,:nocc, :nocc]
        Lvv = kLij[:,:,nocc:, nocc:]
        for k in range(nklist):
            ...
            km = kidx_r[kn]
            Hr1[kn,:] -= (1/nkpts) * einsum('Pij, PQ, Qab, jb->ia', Loo[kn,:].conj(), eps_body_inv, Lvv[kn,:], r1[km,:])
    if bse.singlet:
        Lij, eps_body_inv, kidx_r = make_imds(bse.gw, 0)
        Lov = Lij[:,:,:nocc, nocc:]
        #kL is (0,0,0) 
...
def make_imds(gw, kL):
    ...
    return np.asarray(qkLij)[0], qeps_body_inv[0], all_kidx_r[0]
...
# don't modify the following attributes, they are not input options
    ...
    def get_diag(self):
        ...
        kLij, eps_body_inv, kidx_r = make_imds(bse.gw, 0)
        Loo = kLij[:,:,:nocc,:nocc]
        Lov = kLij[:,:,:nocc,nocc:]
        Lvv = kLij[:,:,nocc:,nocc:]
