...
def matvec(bse, r, orbs):
    ...
    Lij, eps_body_inv, kidx_r = make_imds(bse.gw, 0, orbs)
    Lov = Lij[:,:, bse.mf_nocc-nocc:bse.mf_nocc, bse.mf_nocc:bse.mf_nocc+nvir]
    ...
    for kL in range(nkpts):
        kLij, eps_body_inv, kidx_r = make_imds(bse.gw, kL, orbs)
        Loo = kLij[:,:, bse.mf_nocc-nocc:bse.mf_nocc, bse.mf_nocc-nocc:bse.mf_nocc]
        Lvv = kLij[:,:, bse.mf_nocc:bse.mf_nocc+nvir, bse.mf_nocc:bse.mf_nocc+nvir]
        for k in range(nklist):
            kn = kptlist[k]
            # Find km that conserves with kn and kL (-km+kn+kL=G)
            km = kidx_r[kn]
            # WARNING: Change back! TDA
            Hr1[kn,:] -= (1/nkpts) * einsum('Pij, Pab, jb->ia', Loo[kn,:].conj(), Lvv[kn,:], r1[km,:])
            # Hr1[kn,:] -= (1/nkpts) * einsum('Pij, PQ, Qab, jb->ia', Loo[kn,:].conj(),\
                                            # eps_body_inv, Lvv[kn,:], r1[km,:])
    if bse.singlet:
        #kL is (0,0,0) 
        #should be already shifted back to 0 if shifted kmesh
        for k in range(nklist):
            kn = kptlist[k]
            Hr1[kn,:] += (2/nkpts) * einsum('Qia, qQjb,qjb->ia', Lov[kn].conj(), Lov, r1)
    if bse.TDA:
        return Hr1.ravel()
...
def make_imds(gw, kL, orbs):
    ...
    return np.array(qkLij)[0], qeps_body_inv[0], all_kidx_r[0]
...
# don't modify the following attributes, they are not input options
    ...
    def kernel(self, nstates=None, orbs=None):
        ...
        nkpts = self.nkpts
        mem_incore = (2*2*nkpts*nmo**2*naux) * 16/1e6
    ...
    def gen_matvec(self, orbs):
        ...
        diag = self.get_diag(orbs)
        matvec = lambda xs: [self.matvec(x, orbs) for x in xs]
        return matvec, diag

    def get_diag(self, orbs):
        nocc = sum([x < self.mf_nocc for x in orbs])
        nmo = len(orbs)
        nvir = nmo - nocc
        nkpts = self.nkpts
        ...
        kLij, eps_body_inv, kidx_r = make_imds(self.gw, 0, orbs)
