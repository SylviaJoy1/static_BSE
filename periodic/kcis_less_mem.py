...
def matvec(bse, r, orbs):
    ...
    Lij, kidx_r = make_imds(bse.gw, 0, orbs)
    ...
    for kL in range(nkpts):
        kLij, kidx_r = make_imds(bse.gw, kL, orbs)
...
def make_imds(gw, kL, orbs):
    ...
    return np.array(qkLij)[0], all_kidx_r[0]
...
# don't modify the following attributes, they are not input options
    ...
    def get_diag(self, orbs):
        ...
        kLij, kidx_r = make_imds(self.gw, 0, orbs)
...
if __name__ == '__main__':
    ...
    print('ekS (eV)', 27.2114*ekS)
    
    mybse.singlet=False
    conv, excitations, ekT, xy = mybse.kernel(nstates=_nstates, orbs=bse_orbs)
    
    from pyscf.pbc import tdscf
    # mytd = tdscf.krks.TDDFT(mf)
    mytd = tdscf.krks.TDA(mymf) #mf.TDA()
    mytd.verbose = 7
    mytd.run(nstates = _nstates)
